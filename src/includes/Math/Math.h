...
namespace qw
{
	...
	namespace Math
	{
		...
		template< class T1, class T2 > std::enable_if_t< std::is_arithmetic_v< T1 > && std::is_arithmetic_v< T2 >, T1 >
			pow ( const T1 _val, const T2 _exp )
			{
				typedef common_float_type_t< T1, T2 > type;
				return static_cast< T1 >( ::pow( type( _val ), type( _exp ) ) );
			}

		template< class T > enable_arithmetic_t< T >
			cos ( const T _val )
			{
