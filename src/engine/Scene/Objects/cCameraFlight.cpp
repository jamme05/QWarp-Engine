...
namespace qw::Object
{
	cCameraFlight::cCameraFlight( std::string _name, const float _movement_speed, const float _rotation_speed )
	: cCamera( std::move( _name ) )
	...
	void cCameraFlight::update( void )
	{
		...
		m_transform.getPosition() += Time::Delta * m_speeds.x * ( m_transform.getWorldRight() * m_position.x + m_transform.getWorldUp() * m_position.y + m_transform.getWorldFront() * m_position.z );
		m_transform.getRotation() += Time::Delta * m_speeds.y * cVector3f{ m_rotation.x, m_rotation.y, 0.0f };
		m_transform.update();
		cCamera::update();
	} // update
