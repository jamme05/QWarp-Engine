...
namespace qw
{
	...
	{
		auto itr = m_asset_name_map.find( _name_hash );
		if( itr != m_asset_name_map.end() )
			return itr->second;

		return nullptr;
	} // getAssetByName
	auto cAssetManager::getAssetsByName( const hash<std::string>& _name_hash ) -> std::vector<cShared_ptr<Asset_t>>
	{
		std::vector< cShared_ptr< Asset_t > > assets;
	...
	Asset_id_t cAssetManager::registerAsset( const cShared_ptr< Asset_t >& _asset )
	{
		// TODO: Maybe make a new way to generate ids.
		const Asset_id_t id  = m_assets.size();
		m_assets[ id ]       = _asset;
		_asset->m_id         = id;
		m_asset_name_map.insert( { _asset->getHash(), _asset } );

		return id;
	} // registerAsset

	void cAssetManager::loadFile( const std::filesystem::path& _path )
	{
		const auto extension_hash = hash< std::string >( _path.extension().c_str() );
	...
	auto cAssetManager::loadGltfFile( const std::filesystem::path& _path ) -> std::vector< cShared_ptr< Asset_t > >
	{
		...
		for( auto& mesh : asset.meshes )
		{
			if( auto mesh_asset = handleGltfMesh( asset, mesh ) )
				assets.emplace_back( std::move( mesh_asset ) );
		}
	...
	void cAssetManager::loadEmbedded( void )
	{
		...
		Graphics::cRenderer::m_white_texture = Assets::cTexture::create( "White", white_texture.data(), resolution, Assets::cTexture::eFormat::kRGBA8, true );
	} // loadEmbedded
