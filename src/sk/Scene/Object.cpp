...
{
    _component->SetObject( get_shared() );
...
void sk::Object::cObject::RemoveComponent( const cShared_ptr< iComponent >& _component )
{
    ...
    if( _component->getClassTypeInfo() == kTypeInfo< Components::cMeshComponent > )
    {
        if( const auto itr = std::ranges::find( m_mesh_components_, cShared_ptr{ _component }.Cast< Components::cMeshComponent >() ); itr != m_mesh_components_.end() )
            m_mesh_components_.erase( itr );
    }
    else
    {
        for( auto [ fst, lst ] = m_components_.equal_range( _component->getClassType() ); fst != lst; ++fst )
        {
